...
#             self.sp_kmax[fs_sample_name] = self.kmax_spline[ll]
    ...
    def logp(self,**params_values):
        ...
        class_err = pp.get_result('class_err')
        if class_err:
            lnL = -1.0e4
...
class Taylor_pk_theory_zs(Theory):
    ...
    def get_can_provide(self):
        ...
        return ['taylor_pk_ell_mod','zPars','class_err']
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        cosmopars = [w0,wa,omega_b, omega_cdm, hub, logA]
...
#             p4ktable = taylor_approximate(cosmopars, x0s, derivs4, order=4)
            ...
            try:
                ...
                print('class_error, params are:',cosmopars)
                p0ktable, p2ktable, p4ktable, sig8, Dz, fz = np.ones((len(kv),19)),np.ones((len(kv),19)),np.ones((len(kv),19)),1.,1.,1.
                class_err = True
                message = np.array(['{}:'.format(cosmopars) + str(e)])
                outpath = '/pscratch/sd/m/mmaus/DESI_velocileptors_redef/output_logs/'
                with open(outpath + 'class_err_{:.2f}_{:.2f}.txt'.format(cosmopars[0],cosmopars[1]), "w") as text_file:
                    text_file.write("Erro message: {}".format(message))
