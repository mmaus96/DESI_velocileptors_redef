...
class PkLikelihood(Likelihood):
	...
	# Load the linear parameters of the theory model theta_a such that
        ...
        self.mspl_names = [ 'M%d'%(ii) for ii in range(self.n_spl)]
        self.qspl_names = [ 'Q%d'%(ii) for ii in range(self.n_spl)]
        self.hspl_names = [ 'H%d'%(ii) for ii in range(self.n_spl)]
...
#             self.sp_kmax[fs_sample_name] = self.kmax_spline[ll]
    ...
    def W3(self,x):
        ...
        ret[ii2] = ( (2 - np.abs(x))**3 / 6 )[ii2]

        ret[ii1] = ( (4 + 3 * (np.abs(x)-2) * x**2) /6 )[ii1]
    ...
    def logp(self,**params_values):
        ...
        self.detFish = 0.5 * np.trace(logm(self.Lab)) # np.log( np.linalg.det(self.Lab) )
        if not self.optimize:
            if self.jeff:
                lnL += 0.5 * self.Nlin * np.log(2*np.pi)
            else:
                lnL += - 0.5 * np.trace(logm(self.Lab)) + 0.5 * self.Nlin * np.log(2*np.pi) # volume factor from the determinant
    ...
    def fs_predict(self, fs_sample_name,zfid, thetas=None):
       ...
       # Instead of calling the linear parameters directly we will now analytically marginalize over them
        ...
        if thetas is None:
            ...
            Mspl = [self.linear_param_means[param_name + '_' + fs_sample_name] for param_name in self.mspl_names]
            Qspl = [self.linear_param_means[param_name + '_' + fs_sample_name] for param_name in self.qspl_names]
            if self.hexa:
                alp4_tilde = self.linear_param_means['alpha4_' + fs_sample_name]
                sn4 = self.linear_param_means['SN4_' + fs_sample_name]
                Hspl = [self.linear_param_means[param_name + '_' + fs_sample_name] for param_name in self.hspl_names]
            else: 
                alp4_tilde,sn4 = 0.,0.
                Hspl = [0. for n in range(self.n_spl)]
        else:
            ...
            Mspl = [thetas[param_name + '_' + fs_sample_name] for param_name in self.mspl_names]
            Qspl = [thetas[param_name + '_' + fs_sample_name] for param_name in self.qspl_names]
            if self.hexa:
                alp4_tilde = thetas['alpha4_' + fs_sample_name]
                sn4 = thetas['SN4_' + fs_sample_name]
                Hspl = [thetas[param_name + '_' + fs_sample_name] for param_name in self.hspl_names]
            else: 
                alp4_tilde,sn4 = 0.,0.
                Hspl = [0. for n in range(self.n_spl)]
        ...
        #Add broadband cubic splines
        N = self.n_spl-1
        # Delta = 0.06
        bases = [self.W3(kv/self.delta_spl + 1)] + [self.W3(kv/self.delta_spl - n) for n in range(N) ]
        bases = np.array(bases)
        # print(bases)
        
        P0_spl = np.sum( np.array(Mspl)[:, None] * bases, axis=0 )
        P2_spl = np.sum( np.array(Qspl)[:, None] * bases, axis=0 )
        P4_spl = np.sum( np.array(Hspl)[:, None] * bases, axis=0 )
