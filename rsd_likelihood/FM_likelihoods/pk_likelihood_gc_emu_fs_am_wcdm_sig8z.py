...
class PkLikelihood(Likelihood):
    ...
    def logp(self,**params_values):
        ...
        if self.ext_prior:
            ...
            lnL += -0.5*np.dot(diff_prior,np.dot(self.prior_cinv,diff_prior))
            
        if self.theta_s:
            theta_star = pp.get_param('theta_star')
            theta_star_true = pp.get_param('theta_star_true')
            lnL += -0.5*(theta_star_true - theta_star)**2/self.theta_s_prior**2
    ...
    def fs_predict(self, fs_sample_name,zfid, thetas=None):
       ...
       # Instead of calling the linear parameters directly we will now analytically marginalize over them
        ...
        bias = [b1, b2, bs, b3]
        cterm = [alp0/(sig8_z**2),alp2/(sig8_z**2),alp4/(sig8_z**2),alp6/(sig8_z**2)]
        stoch = [sn0, sn2, sn4]
    ...
    def fs_observe(self,tt,fs_sample_name):
        ...
        if np.any(np.isnan(thy)) or np.max(thy) > 1e8:
            ...
            print("NaN's encountered. Parameter values are: {}".format([hub,sig8,OmM]))
...
class Taylor_pk_theory_zs(Theory):
    ...
    def initialize(self):
        ...
        self.theta_star_true = self.compute_theory.theta_star
        # Load clustering
        self.taylors_pk = {}
        self.s8_emus = {}
    ...
    def get_can_provide_params(self):
        return ['sigma8','omegam','theta_star','theta_star_true']
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        theta_star = self.compute_theory.compute_theta_star_w0waCDM([w,0.,omega_b, omega_cdm, hub, logA])
        state['derived']['theta_star'] = theta_star
        state['derived']['theta_star_true'] = self.theta_star_true
