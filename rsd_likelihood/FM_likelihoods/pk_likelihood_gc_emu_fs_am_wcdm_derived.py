...
class PkLikelihood(Likelihood):
    ...
    def calculate(self,state,want_derived=True,**params_values):
        ...
        self.bf_thetas = np.einsum('ij,j', self.Lab_inv, self.Va)
        
        startii =0
        for ii,fs_sample_name in enumerate(self.fs_sample_names):
            if self.hexa:
                alp0,alp2,alp4 = self.bf_thetas[startii:startii+3]
                sn0,sn2,sn4 = self.bf_thetas[startii+3:startii+6]
                startii += 6
            else:
                alp0,alp2 = self.bf_thetas[startii:startii+2]
                sn0,sn2 = self.bf_thetas[startii+2:startii+4]
                startii += 4
                
            
            state['derived']['alpha0_'+fs_sample_name] = alp0
            state['derived']['alpha2_'+fs_sample_name] = alp2
            state['derived']['SN0_'+fs_sample_name] = sn0
            state['derived']['SN2_'+fs_sample_name] = sn2
            if self.hexa:
                state['derived']['alpha4_'+fs_sample_name] = alp4
                state['derived']['SN4_'+fs_sample_name] = sn4
...
#         self.bf_thetas = np.einsum('ij,j', self.Lab_inv, self.Va)
#         if self.hexa:
#             alp0,alp2,alp4 = self.bf_thetas[:3]
#             sn0,sn2,sn4 = self.bf_thetas[3:]
#         else:
#             alp0,alp2 = self.bf_thetas[:2]
#             sn0,sn2 = self.bf_thetas[2:]
            
        
#         # derived = {'alpha0_LRG': alp0 ,\
#         #           'alpha2_LRG': alp2 ,\
#         #           # 'alpha4_LRG': alp4 ,\
#         #           'SN0_LRG': sn0 ,\
#         #           'SN2_LRG': sn2 ,\
#         #           }
#         state['derived']['alpha0_LRG'] = alp0
#         state['derived']['alpha2_LRG'] = alp2
#         state['derived']['SN0_LRG'] = sn0
#         state['derived']['SN2_LRG'] = sn2
#         if self.hexa:
#             state['derived']['alpha4_LRG'] = alp4
#             state['derived']['SN4_LRG'] = sn4
...
class Taylor_pk_theory_zs(Theory):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        for zfid in self.zfids:
            ...
            sig8_emu = compute_zpars.compute_sig8([w,OmM,hub,logA],order = 5)
            Dz_emu = compute_zpars.compute_Dz([w,OmM,hub,logA],order = 5)
            fz_emu = compute_zpars.compute_fz([w,OmM,hub,logA],order = 5)
            sig8z_emu =  sig8_emu*Dz_emu
            zPars[zstr] = [Dz_emu,fz_emu]
            
        #state['sigma8'] = sig8
        # state['derived'] = {'sigma8': sig8,'omegam':OmM}
        state['derived']['sigma8'] = sig8_emu
