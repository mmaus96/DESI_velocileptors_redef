...
class JointLikelihood(Likelihood):
    ...
    def initialize(self):
        ...
        for ll, bao_sample_name in enumerate(self.bao_sample_names):
            self.Rs[bao_sample_name] = self.Rsmooth[ll]
...
#             startii += self.rdats[bao_sample_name].size
    ...
    def bao_predict(self, bao_sample_name,zfid, thetas=None):
        ...
        sigmas = (pp.get_param('Sigpar_' +  bao_sample_name),\
                 pp.get_param('Sigperp_' +  bao_sample_name),\
                 pp.get_param('Sigs_' +  bao_sample_name))
        
        pkclass = pp.get_result('pkclass')
        zPars = pp.get_result('zPars')
        R = self.Rs[bao_sample_name]
        ...
        rvec, xi0table, xi2table,sig8,Dz,fz =compute_xiell_tables(cosmopars, z=zfid, fid_dists= zPars[zstr][2], R=R, rmin=60, rmax=160, dr=0.5,pkclass = pkclass,sigs=sigmas)
...
#             taylors_xi['derivs_xi2'] = derivs_x2
    ...
    def get_can_provide(self):
        ...
        return ['taylor_pk_ell_mod','pkclass','zPars']
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        pkclass =  compute_pkclass(cosmopars)
        sig8 = pkclass.sigma(8./hub,z=0)    
        ll=0
        for zfid,R in zip(self.zfids,self.Rsmooth):
            ...
            # sigmas = (pp.get_param('Sigpar_' +  self.bao_sample_names[ll]),\
            #          pp.get_param('Sigperp_' +  self.bao_sample_names[ll]),\
            #          pp.get_param('Sigs_' +  self.bao_sample_names[ll]))
            
            # rv, xi0table, xi2table,sig8,Dz,fz =compute_xiell_tables(cosmopars, z=zfid, fid_dists= self.fid_dists[zstr], R=R, rmin=60, rmax=160, dr=0.5,pkclass = pkclass,sigs=sigmas)
...
#             derivs_xi2 = self.taylors_xi[zstr]['derivs_xi2']
            zPars[zstr] = [Dz,fz,self.fid_dists[zstr]]
