...
class shapefit_2_wCDM(Likelihood):
    zfids: list
    # covfn: str
    chains_fn: list
    basedir: str
    ...
    def logp(self,**params_values):
        ...
        for kk,z in enumerate(self.zfids):
            zstr = "%.2f" %(z)
            diff = self.par_means[zstr] - theory_pars[zstr]
        
            chi2 += np.dot(diff,np.dot(self.cinv[zstr],diff))
            print(theory_pars[zstr],chi2)
        ...
        if self.ext_prior:
            ...
            d_prop = np.array([omega_b,omega_cdm])
    ...
    def loadData(self):
        ...
        self.cinv = {}
        self.par_means = {}
        for ii,fn in enumerate(self.chains_fn):
            z = self.zfids[ii]
            zstr = "%.2f" %(z)
            if self.unpack_chains:
                samples = loadMCSamples(self.basedir + fn,no_cache=True, \
                ...
                if self.shapefit:
                    ...
                    covmat = samples.cov(pars=['f_sig8','apar','aperp','m'])
                    # m_dat = p['m']
                    ...
                    covmat = samples.cov(pars=['f_sig8','apar','aperp'])
                ...
                self.cinv[zstr] = np.linalg.inv(covmat)
                print(self.cinv[zstr])
                self.par_means[zstr] = par_means
            else:
                ...
                self.cinv[zstr] = deepcopy(cov_inv)
                self.par_means[zstr] = np.mean(chain,axis=0)
...
class Compute_theory_pars(Theory):
    ...
    def initialize(self):
        ...
        "Omega_k": float(0.0),
        "output": "mPk",
        "P_k_max_1/Mpc": float(5.0),
        "z_max_pk": np.max(self.zfids) + 0.5
         })
...
#         
            ...
            # fsigma8_fid = fid_class.scale_independent_growth_factor_f(zfid)*fid_class.sigma(8.0/h_fid, zfid)
            sigma8_fid = fid_class.sigma(8.0/h_fid, zfid)
            # Pk_ratio_fid = EHpk_fid
            print(zfid, sigma8_fid)

            self.fiducial_all[zstr] = [h_fid, Hz_fid, rd_fid, Chiz_fid, Amp_fid, transfer_fid, sigma8_fid]
    ...
    def calculate(self, state, want_derived=False, **params_values_dict):
        ...
        state['theory_pars'] = {}
        for jj,z in enumerate(self.zfids):
            zstr = "%.2f" %(z)
            ...
            #The fiducial values. 
            h_fid, Hz_fid, rd_fid, Chiz_fid, Amp_fid, transfer_fid, sigma8_fid = self.fiducial_all[zstr]
            ...
            if self.shapefit:
                ...
                theory_pars = [theo_fAmp*sigma8_fid, theo_apara, theo_aperp, theo_mslope]
            else:
                ...
                # theory_pars = [ theo_fAmp*sigma8_fid*fAmp_mfac, theo_apara, theo_aperp]

            state['theory_pars'][zstr] = theory_pars
        # sigma8_0 = pkclass.sigma(s*8.0 / pkclass.h(), 0.)
        sigma8_0 = pkclass.sigma(8.0 / pkclass.h(), 0.)
        state['derived'] = {'omegam': OmegaM, 'sigma8': sigma8_0}