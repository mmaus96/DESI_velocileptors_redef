...
from compute_xiell_tables_recsym import compute_xiell_tables_SF
...
#             self.sp_kmax[fs_sample_name] = self.kmax_spline[ll]
    ...
    def loadData(self):
        ...
        print('----------------------------------------')
        print('--------Creating Joint Covariance-------')
        print('----------------------------------------')
        joint_cov = np.zeros((len(self.dd),len(self.dd)))
        joint_startii = 0
        bao_count = 0
        for ss, fs_sample_name in enumerate(self.fs_sample_names):
            print(ss)
            # Now load the covariance matrix.
            cov = np.loadtxt(self.basedir + self.covfns[ss])/self.cov_fac
            # this is going to tell us how many indices to skip to get to the nth multipole
            startii = 0
            print('indices: joint={}, single={}, bao={}'.format(joint_startii,startii,bao_count))
            ...
            if fs_sample_name in self.bao_sample_names:
                rcut = (self.rdats[fs_sample_name] < self.bao_rmins[bao_count])\
                              | (self.rdats[fs_sample_name] > self.bao_rmaxs[bao_count])
            ...
            joint_cov[joint_startii : joint_startii + startii,joint_startii : joint_startii + startii] = cov
    
            joint_startii += startii
            print('indices: joint={}, single={}, bao={}'.format(joint_startii,startii,bao_count))
            print('')
...
#             startii += self.rdats[bao_sample_name].size
    ...
    def fs_predict(self, fs_sample_name,zfid, thetas=None):
        ...
        #
        sig8_z = pp.get_param('sig8_z')
        fsig8 = pp.get_param('f_sig8')
        f_z = fsig8/sig8_z
    ...
    def fs_observe(self,tt,fs_sample_name):
        ...
        kv = self.pyp_kin[fs_sample_name]
        ...
        # wide angle
        convolved_model = np.matmul(self.matMs[fs_sample_name], thy )
        ...
        convolved_new = np.array([])
        nout = len(self.pyp_kout[fs_sample_name])
        for i in range(3):
        # i=0
            convolved_new = np.concatenate((convolved_new,np.concatenate(([0.],convolved_model[i*nout:i*nout+nout]))))
            if i<2:
                convolved_new = np.concatenate((convolved_new,np.concatenate(([0.],convolved_model[i*nout:i*nout+nout]))))
        
        # keep only the monopole and quadrupole
        convolved_model = convolved_new[self.fitiis[fs_sample_name]]
...
class Taylor_pk_theory_zs(Theory):
    ...
    def initialize(self):
        ...
        hfid = self.compute_theory.fid_class.h()
        self.sig8_z = self.compute_theory.fid_class.sigma(8./hfid,self.zfids[0])
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        for zfid in self.zfids:
            ...
            p0ktable = taylor_approximate(cosmopars, x0s, derivs0, order=self.order)
            p2ktable = taylor_approximate(cosmopars, x0s, derivs2, order=self.order)
            p4ktable = taylor_approximate(cosmopars, x0s, derivs4, order=self.order)
            
            ptables[zstr] = (kv, p0ktable, p2ktable, p4ktable)
            
            rv, xi0table, xi2table =compute_xiell_tables_SF(cosmopars,self.compute_theory.fid_class, z=zfid, R=15., rmin=60, rmax=160, dr=0.5)
            
            fsig_s8 = self.compute_theory.compute_fsigma_s8(cosmopars,zfid)
...
#             xi2table = taylor_approximate(cosmopars, x0s, derivs_xi2, order=4)
        ...
        #state['sigma8'] = sig8
        state['derived'] = {'fsig_s8': fsig_s8,'sig8_z': self.sig8_z}
