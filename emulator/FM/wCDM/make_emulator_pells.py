...
x0s = [-1.0,0.02237, 0.1200,0.6736, 3.036394]; Nparams = len(x0s) # these are chosen to be Abacus fiducial values
...
for nn, iis in enumerate(zip(*Inds)):
    if nn%mpi_size == mpi_rank:
        ...
        print(coord,iis)
        try:
            p0, p2, p4, _ = compute_pell_tables(coord,z=z,fid_dists=fid_dists)
        except:
...
# Now save:
outfile = basedir + 'emu/abacus_z_%.2f_pkells_desilike.json'%(z)
