...
x0s = [-1.0,0.02237, 0.1200,0.6736, 3.036394]; Nparams = len(x0s) # these are chosen to be roughly at the BOSS best fit value
dxs = [0.1,0.0015, 0.01,0.03, 0.05]
...
for nn, iis in enumerate(zip(*Inds)):
    if nn%mpi_size == mpi_rank:
        ...
        print(coord,iis)
        s8_gridii[iis],Dz_gridii[iis],fz_gridii[iis],sig8z = compute_sigma8(coord,z)
        print((s8_gridii[iis]*Dz_gridii[iis] - sig8z)/sig8z)
...
if mpi_rank == 0:
    ...
    #
    outfile = basedir+'emu/abacus_s8_wcdm_z%.2f_desilike.json'%(z)
